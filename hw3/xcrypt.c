...
int copy_xcrypt_data_to_kernel(xcrypt *user_param, xcrypt *kernel_param)
{
	...
	if (rc) {
		pr_err("En/Decryption: Copying of input file failed.\n");
		goto free_infile;
	}
	...
	if (rc) {
		pr_err("En/Decryption: Copying of output file failed.\n");
		goto free_outfile;
	}
	...
	if (rc) {
		pr_err("En/Decryption: Copying of cipher name failed.\n");
		goto free_cipher;
	}
	...
	if (rc) {
		pr_err("En/Decryption: Copying of Passphrase failed.\n");
		goto free_keybuf;
	}
	...
	rc = copy_from_user(&kernel_param->keylen, &user_param->keylen,
		sizeof(int));
	if (rc) {
		pr_err("En/Decryption: Copying of key buffer length failed.\n");
		goto free_keybuf;
	}

	rc = copy_from_user(&kernel_param->flag, &user_param->flag,
		sizeof(int));
	if (rc) {
		pr_err("En/Decryption: Copying of encryption/decryption flag "
			"failed.\n");
		goto free_keybuf;
	}
...
int getMD5Hash(char *str, u8 *md5_hash)
{
	...
	/*
	 * This MD5_HASH generation algorithm is inspired from symlink_hash(...)
	...
	void *dst, size_t *dst_len, const void *src, size_t src_len)
{
	...
	if (IS_ERR(tfm)) {
		pr_err("crypto_alloc_blkcipher failed! Check if the cipher "
	...
	*dst_len = src_len + zero_padding;
	...
	if (ret < 0) {
		pr_err("crypto_blkcipher_encrypt failed!\n");
		return ret;
	}
	return 0;
}
	...
	void *dst, size_t *dst_len, const void *src, size_t src_len)
{
	...
	if (ret < 0) {
		pr_err("crypto_blkcipher_decrypt failed!\n");
		return ret;
	}
...
int do_xcrypt(xcrypt *xcrypt_obj)
{
			...
			if (vfs_stat(outfilp_lock_name, &stat) != 0) {
				...
				if (infilp_lock && !IS_ERR(infilp_lock)) {
					if (infilp_lock->f_path.dentry != NULL &&
						infilp_lock->f_path.dentry->d_parent->d_inode !=
						NULL) {
	...
	case ENCRYPT:
		...
		while ((bytes = infilp->f_op->read(infilp, read_buffer, PAGE_SIZE,
			...
			if (bytes < PAGE_SIZE) {
				len = bytes + to_pad;
				rc = do_encryption(cipher_algo, xcrypt_obj->keybuf,
					AES_BLOCK_SIZE, write_buffer, &len, read_buffer,
					bytes + to_pad);
			...
			if (rc) {
				pr_err("Failed while encrypting the file.\n");
				goto reset_fs;
			}
		}
		break;
	case DECRYPT:
		...
		if (memcmp(md5_hash, key_buffer, AES_BLOCK_SIZE)) {
			pr_err("Oops. Wrong Key or Cipher Algorithm.\n");
			rc = -EPERM;
			goto reset_fs;
		}
		...
		rc = do_decryption(cipher_algo, xcrypt_obj->keybuf, AES_BLOCK_SIZE,
			...
			2 * AES_BLOCK_SIZE);
	    if (rc) {
			pr_err("Failed while decrypting the padding info.\n");
			goto reset_fs;
	    }
		...
		while ((bytes = infilp->f_op->read(infilp, read_buffer, PAGE_SIZE,
			...
			if (rc) {
				pr_err("Failed while decrypting the file.\n");
				goto reset_fs;
			}
		}
		break;
